...
import CategoryInvestAdd from "@/components/category-invest/CategoryInvestAdd";
import {
  deleteCategoryInvest,
  getCategoryInvests,
  type CategoryInvest,
} from "@/services/CategoryInvestService";
import CategoryInvestUpdate from "@/components/category-invest/CategoryInvestUpdate";
import { useSession } from "next-auth/react";

const PAGE_SIZE = 10;

export default function CategoryInvestTable() {
  const [items, setItems] = useState<CategoryInvest[]>([]);
  const [showAddModal, setShowAddModal] = useState(false);
  const [selectedItem, setSelectedItem] = useState<CategoryInvest | null>(null);
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      setItems(res.items as CategoryInvest[]);
      setTotal(res.total);
      setTotalPages(res.totalPages);
    } catch (err: any) {
      toast.error("Lỗi khi tải danh mục đầu tư", { description: err.message });
    }
  };
