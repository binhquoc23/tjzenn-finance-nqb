...
import {
  updateCategoryInvest,
  type CategoryInvest,
} from "@/services/CategoryInvestService";

interface CategoryInvestUpdateProps {
  item: CategoryInvest; // dữ liệu hiện tại
  onClose: () => void;
  onUpdated?: (updated: CategoryInvest) => void; // callback sau khi cập nhật
}
...
export default function CategoryInvestUpdate({
  item,
  onClose,
  onUpdated,
}: CategoryInvestUpdateProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const updated = await updateCategoryInvest(item.id, payload);

      toast.success("Cập nhật danh mục đầu tư thành công", {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1c1c1e] rounded-xl w-full max-w-md h-[70vh] relative flex flex-col">
        ...
        {/* Form */}
        <form
          id="update-category-invest-form"
        ...
        >
          <div>
            ...
            <input
              ...
              required
              disabled={loading}
              placeholder="Ví dụ: Cổ phiếu, Trái phiếu, Crypto ..."
            />
          </div>
        ...
        <div className="sticky bottom-0 z-10 bg-[#1c1c1e] px-6 py-4 border-t border-white/10">
          <button
            type="submit"
            form="update-category-invest-form"
