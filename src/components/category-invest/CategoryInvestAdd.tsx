...
import { createCategoryInvest } from "@/services/CategoryInvestService";
...
interface CategoryInvestAddProps {
  onClose: () => void;
  onAdd?: (cat: CategoryInvest) => void;
}
...
export default function CategoryInvestAdd({
  onClose,
  onAdd,
}: CategoryInvestAddProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const row = await createCategoryInvest({
        name: form.name.trim(),
      ...
      toast.success("Tạo danh mục đầu tư thành công", {
        description: `“${row.name}” đã được thêm.`,
      });

      onAdd?.(row as CategoryInvest);
      onClose();
    } catch (err: any) {
