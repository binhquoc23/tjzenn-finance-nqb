...
import { createCategoryExpense } from "@/services/CategoryExpenseService";
...
interface CategoryExpenseAddProps {
  onClose: () => void;
  onAdd?: (cat: CategoryExpense) => void;
}

const PRESET_COLORS = [
  "#FFFFFF", // trắng - nổi bật tuyệt đối
  "#EF4444", // đỏ tươi
  "#F97316", // cam
  "#F59E0B", // vàng đậm
  "#22C55E", // xanh lá tươi
...
export default function CategoryExpenseAdd({
  onClose,
  onAdd,
}: CategoryExpenseAddProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const row = await createCategoryExpense({
        name: form.name.trim(),
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1c1c1e] rounded-xl w-full max-w-md h-[70vh] relative flex flex-col">
        ...
        >
          <div>
            ...
            <input
              ...
              required
              disabled={loading}
              placeholder="Ví dụ: Giải trí, Thiết yếu, ..."
            />
          </div>
        ...
        <div className="sticky bottom-0 z-10 bg-[#1c1c1e] px-6 py-4 border-t border-white/10">
          <button
            type="submit"
            form="add-category-expense-form"
