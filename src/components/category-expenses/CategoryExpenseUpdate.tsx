...
interface CategoryExpenseUpdateProps {
  item: CategoryExpense; // dữ liệu hiện tại
  onClose: () => void;
  onUpdated?: (updated: CategoryExpense) => void; // callback sau khi cập nhật
}
...
export default function CategoryExpenseUpdate({
  item,
  onClose,
  onUpdated,
}: CategoryExpenseUpdateProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const updated = await updateCategoryExpense(item.id, payload);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1c1c1e] rounded-xl w-full max-w-md h-[70vh] relative flex flex-col">
        ...
        {/* Form */}
        <form
          id="update-category-expense-form"
        ...
        <div className="sticky bottom-0 z-10 bg-[#1c1c1e] px-6 py-4 border-t border-white/10">
          <button
            type="submit"
            form="update-category-expense-form"
