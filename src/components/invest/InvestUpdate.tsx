// app/(wherever)/components/InvestUpdate.tsx
"use client";
...
import { type Invest, updateInvest } from "@/services/InvestService";

interface InvestUpdateProps {
  invest: Invest;
  onClose: () => void;
...
export default function InvestUpdate({
  invest,
  onClose,
  onUpdate,
}: InvestUpdateProps) {
  ...
  const [form, setForm] = useState<{
    ...
    name: invest.name || "",
    category: invest.category != null ? String(invest.category) : "",
    amount: String(invest.amount ?? ""),
    amountDisplay: formatVND(String(invest.amount ?? "")),
    dateYMD: invest.date || "",
    dateLabel: ymdToLabel(invest.date || ""),
    note: invest.note || "",
  }));
  ...
  const [selectedCategory, setSelectedCategory] = useState<Option | null>(
    invest.category != null
      ? { value: String(invest.category), label: "" } // label sẽ cập nhật sau
      : null
  );
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (invest.category != null) {
          const found = opts.find((o) => o.value === String(invest.category));
          if (found) setSelectedCategory(found);
        }
      } catch (e) {
        toast.error("Không tải được danh mục đầu tư");
      }
    })();
  }, [userId, invest.category]);
  ...
  const handleCreateCategory = async (inputLabel: string) => {
    ...
    try {
      const created = await createCategoryInvest({
        name,
        user_id: userId,
        color: "#FFFFFF",
      });
      ...
      toast.success("Đã tạo danh mục đầu tư mới", {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!dateYMD) throw new Error("Ngày đầu tư không được để trống");
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1c1c1e] rounded-xl w-full max-w-6xl h-[90vh] relative flex flex-col">
        ...
        <div className="sticky top-0 z-10 bg-[#1c1c1e] p-6 border-b border-white/10">
          <h2 className="text-2xl font-bold text-white">Sửa khoản đầu tư</h2>
          <button
        ...
        >
          ...
          {/* date */}
          <div>
            <label className="block mb-1 text-white">Ngày đầu tư</label>
            <div className="relative">
              <input
                readOnly
                value={form.dateLabel}
                className="w-full px-4 py-2 rounded-lg bg-black text-white border border-gray-600"
                aria-label="Ngày đầu tư (dd/mm/yyyy)"
              />
              <input
                ref={dateRef}
                type="date"
          ...
          <div>
            <label className="block mb-1 text-white">
              Ghi chú{" "}
              <span className="text-green-600">(Có thể đính kèm hoá đơn)</span>
            </label>
            <Editor
