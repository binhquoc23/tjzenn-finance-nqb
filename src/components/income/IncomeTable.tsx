...
function parseAmountToNumber(v: any): number | null {
  ...
  // giữ số + dấu - (nếu có) và dấu phẩy/chấm để xử lý
...
function parseDateFromCell(v: any): string | null {
  ...
  // 1) Excel date dạng number (serial)
  if (typeof v === "number" && Number.isFinite(v)) {
    // XLSX.SSF.parse_date_code chuyển serial -> {y,m,d}
...
export default function IncomeTable() {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      // 1) Tải TẤT CẢ bản ghi theo filter hiện tại (không chỉ 1 trang)
      let pageIdx = 1;
      const pageSizeExport = 2000; // to có chút để giảm vòng lặp
      const all: Income[] = [];
      // Dùng các filter hiện tại
      while (true) {
        const res = await getIncomes({
          page: pageIdx,
      ...
      (catRes.items as CategoryIncome[]).forEach((c) =>
      ...
      // Body: kẻ bảng đầy đủ 3 cột + căn TRÁI
      for (let r = 3; r < aoaCat.length; r++) {
        for (let c = 0; c <= 2; c++) {
          ...
          wsCat[addr].s = {
            ...(wsCat[addr].s || {}),
            ...bodyCellStyle, // đã có border mỏng
            alignment: {
              horizontal: "left",
              vertical: "center",
              shrinkToFit: true,
      ...
      // ==== Sheet B: Chi tiết ====
      // BỎ cột Ghi chú → chỉ 4 cột: Ngày | Tên | Danh mục | Số tiền (VND)
      const aoaDetail: any[][] = [
        ["Chi tiết Thu nhập", ""],
        [""],
      ...
      // Body: kẻ bảng, căn TRÁI, định dạng ngày & tiền
      const startRow = 3;
      const endRow = 2 + (aoaDetail.length - 2); // đến hết dữ liệu
      ...
      // ÉP width trực tiếp: A nhỏ, B vừa, C TO RÕ RỆT, D to
      const COL_A_MIN = 10; // Ngày
      const COL_B_MIN = 40; // Tên
      const COL_C_MIN = 40; // Danh mục — tăng mạnh
      const COL_D_MIN = 32; // Số tiền
  ...
  const handleImportExcel = async (file: File) => {
    ...
    try {
      ...
      // xác định có header không (Ngày/Tên/Số tiền)
      ...
      const hasHeader =
        ...
        first.some(
          (t) =>
            t.includes("tiền") || t.includes("số tiền") || t.includes("amount")
        );

      const dataRows = hasHeader ? rows.slice(1) : rows;

      // Map cột: nếu có header -> tìm index, nếu không -> A,B,C
      let idxDate = 0,
        idxName = 1,
        idxAmount = 2;
      ...
      const prepared = dataRows
        .map((r, i) => {
          ...
          return {
            rowIndex: hasHeader ? i + 2 : i + 1, // để báo lỗi đúng dòng Excel
            dateYMD,
            name,
            amount,
          };
        })
        .filter((x) => x.name || x.amount || x.dateYMD); // bỏ dòng trắng
      ...
      // import từng dòng (sequential để đỡ spam DB)
      let okCount = 0;
      for (const r of prepared) {
        await createIncome({
          name: r.name,
          category: null, // ✅ bỏ trống danh mục
          amount: r.amount!,
          date: r.dateYMD!,
          note: null,
          user_id: userId,
      ...
      toast.success("IMPORT THÀNH CÔNG", {
        description: `Đã thêm ${okCount} thu nhập.`,
      });
      ...
      setImporting(false);
      if (importRef.current) importRef.current.value = ""; // cho phép chọn lại cùng file
    }
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2 gap-3">
          ...
          <button
            ...
            title={
              !userId
                ? "Vui lòng đăng nhập"
                : "Import từ Excel (Ngày | Tên | Số tiền)"
            }
            type="button"
          >
      ...
      <div className="overflow-x-auto">
        ...
        {showAddModal && (
          <IncomeAdd
            ...
            onAdd={() => {
              fetchIncomes(); // reload list sau khi thêm mới
            }}
            onCategoryCreated={(c) => {
